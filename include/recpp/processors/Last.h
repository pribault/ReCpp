...
{
	/**
	 * @class Last Last.h <recpp/processors/Last.h>
	 * @brief {@link rscpp::Processor} that will subscribe to a given {@link rscpp::Publisher}, retrieve the last element and emit it. If the
	 ...
	 */
	template <typename T>
	class Last : public rscpp::Processor<T, T>
	{
	...
	public:
		/**
		 * @brief Construct a new {@link Last} instance.
		 *
		 * @param publisher The source {@link rscpp::Publisher} the {@link Last} {@link rscpp::Processor} subscribes to.
		 */
		explicit Last(const rscpp::Publisher<T> &publisher);
	};
} // namespace recpp::processors

#include <recpp/processors/inl/Last.inl>
